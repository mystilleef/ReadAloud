...
async function run() {
  ...
  if (!bumpType || !['major', 'minor', 'patch'].includes(bumpType)) {
    ...
    console.log(chalk.yellow('Usage: npm run release -- <major|minor|patch> [--dry-run] [--prerelease]'));
    process.exit(1);
  }

  if (isDryRun) {
    console.log(chalk.yellow('ℹ️ Running in dry-run mode. No changes will be made.'));
  }
  ...
  try {
    ...
    console.error(chalk.red(`❌ Pre-flight check failed: ${error.message}`));
    process.exit(1);
  }
  ...
  if (!newVersion) {
    console.error(chalk.red(`❌ Could not determine new version from ${currentVersion} and bump type ${bumpType}.`));
    process.exit(1);
  }
  ...
  if (isDryRun) {
    ...
    console.log(`  - Run: git commit -m "chore(release): bump version to v${newVersion}"`);
    ...
    await execa('git', ['commit', '-m', `chore(release): bump version to v${newVersion}`]);
  ...
  if (isDryRun) {
    ...
    console.log(`  - Run: gh release create v${newVersion} --title "Release v${newVersion}" --generate-notes`);
  } else {
    ...
    await execa('gh', ['release', 'create', `v${newVersion}`, '--title', `Release v${newVersion}`, '--generate-notes', '--notes', '']);
...
async function checkTools() {
  try {
    ...
    throw new Error('Required tools (git or gh CLI) are not installed or not in PATH.');
  }
...
async function syncWithRemote() {
  ...
  const { stdout: remoteSha } = await execa('git', ['rev-parse', 'origin/main']);

  if (localSha !== remoteSha) {
    throw new Error('Local main branch is not in sync with origin/main. Please pull latest changes.');
  }
